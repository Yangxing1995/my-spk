...
DSMService = "http://192.168.252.219:5000"  # Synology IP Address Format: http://domain:port        demo: https://nimocass.com:80
...
certList = "/webapi/entry.cgi?api=SYNO.Core.Certificate.CRT&method=list&version=1&_sid="  # certList
...
#     print(JsonText)
#     Sid = JsonText['data']['sid']  # 通过JSON获取到的内容
#     Systemstatus = RequestUrl(DSMService + certList + Sid)
#     # SystemInformation = RequestUrl(DSMService + SystemUtilization_head_url + Sid)
...
def upload_files_and_data(key_path, cert_path, inter_cert_path, id_value, desc_value, as_default_value, url):
    """
    上传文件和表单数据到指定的 URL。
    ...
    key_path -- 密钥文件的路径
    cert_path -- 证书文件的路径
    inter_cert_path -- 中级证书文件的路径
    id_value -- 'id'字段的值 ，可以为空
    desc_value -- 'desc'字段的值
    as_default_value -- 是否作为默认证书 true or false
    url -- 请求的目标 URL
    """
    # 打开文件并准备文件参数
    files = {
        'key': (os.path.basename(key_path), open(key_path, 'rb')),
        'cert': (os.path.basename(cert_path), open(cert_path, 'rb')),
        'inter_cert': (os.path.basename(inter_cert_path), open(inter_cert_path, 'rb'))
    }
...
if __name__ == "__main__":
    ...
    response = upload_files_and_data(
        key_path='path/to/your/key.key',
        cert_path='path/to/your/cert.pem',
        inter_cert_path='path/to/your/inter_cert.pem',
        id_value='NMt6Xy',
        desc_value='replace',
        as_default_value='true',
        url= DSMService + certList + Sid
    )

    if response.ok:
        print("上传成功，响应内容：", response.text)
    else:
